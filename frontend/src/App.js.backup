...
const ThriftHub = () => {
  ...
  // Demo data (fallback)
  const featuredItems = [
    { id: 1, name: "Vintage Denim Jacket", price: 35, originalPrice: 89, image: "🧥", category: "Jackets", condition: "Excellent", seller: "VintageVibes", rating: 4.8, discount: 61 },
    { id: 2, name: "Floral Summer Dress", price: 28, originalPrice: 75, image: "👗", category: "Dresses", condition: "Good", seller: "EcoFashion", rating: 4.6, discount: 63 },
    { id: 3, name: "Classic Leather Boots", price: 45, originalPrice: 120, image: "👢", category: "Shoes", condition: "Very Good", seller: "RetroStyle", rating: 4.9, discount: 63 },
    { id: 4, name: "Silk Scarf", price: 15, originalPrice: 45, image: "🧣", category: "Accessories", condition: "Excellent", seller: "LuxeSecond", rating: 4.7, discount: 67 },
    { id: 5, name: "Wool Sweater", price: 32, originalPrice: 85, image: "🧶", category: "Tops", condition: "Good", seller: "CozyFinds", rating: 4.5, discount: 62 },
    { id: 6, name: "Designer Handbag", price: 89, originalPrice: 250, image: "👜", category: "Accessories", condition: "Excellent", seller: "LuxeSecond", rating: 4.9, discount: 64 },
  ];

  const categories = [
    { name: "Clothing", icon: "👔", count: 1234, subcategories: ["Tops", "Bottoms", "Dresses", "Outerwear"] },
    { name: "Shoes", icon: "👠", count: 567, subcategories: ["Sneakers", "Boots", "Heels", "Flats"] },
    { name: "Accessories", icon: "👜", count: 890, subcategories: ["Bags", "Jewelry", "Scarves", "Belts"] },
    { name: "Vintage", icon: "✨", count: 345, subcategories: ["1970s", "1980s", "1990s", "2000s"] },
  ];
                 ...
                 <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-lg hover:shadow-2xl transform hover:-translate-y-4 transition-all duration-500">
                   ...
                   <div className="space-y-2">
                     {category.subcategories.map((sub, subIndex) => (
                       <div key={subIndex} className="text-sm text-slate-500 font-medium">• {sub}</div>
                     ))}
                   </div>
                 </div>
               </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
             {favoriteItems.map((item) => (
               <div key={item.id || item._id} className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300">
                 ...
                 <div className="p-4">
                   <h3 className="font-bold text-lg mb-2">{item.name}</h3>
                   <p className="text-sm text-gray-500 mb-3">{item.condition} • {item.seller || item.sellerName}</p>
                   <div className="flex justify-between items-center">
                     <p className="text-xl font-bold text-indigo-600">${item.price}</p>
                     <button 
                       onClick={() => addToCart(item)}
         ...
         <div className="flex items-center gap-6 mb-8">
           ...
           <div className="flex-1">
             ...
             <p className="text-gray-600 text-lg">
               {user?.role === 'seller' ? 'Verified Seller' : 'Sustainable Fashion Enthusiast'} • 
